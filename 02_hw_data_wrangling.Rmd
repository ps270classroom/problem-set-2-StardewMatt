...
incumbency_table
```
There are 7,014 candidates in the fb_ads dataset.
...
knitr::kable(party_incumbent_promote, digits = 3, col.names = c("Party", "Incumbency", "Average Proportion of Promotional Ads"))
```

On average, Democrat incumbents sponsored the most promotional ads (85%).
...
plot_q3 <- fb_ads |>
  ...
  geom_histogram(stat = "identity", na.rm = TRUE)

plot_q3
```
The plot makes an upside-down pyramid shape (a sort of log graph shape), with most candidates receiving less than 10 million impressions on Facebook.
...
fb_top_plot <- fb_ads |>
  ...
  ggplot(mapping = aes(x = impressions_millions, 
                                       y = fct_reorder(cand_name_last, impressions_millions))) +
  geom_bar(stat = "identity", aes(fill = party), na.rm = TRUE) +
  labs(title = "Top 15 House Candidate Facebook Ad Impressions, 2017-2018",
       x = "Facebook Ad Impressions (in millions)",
       y = "House Candidate Last Name") +
  scale_fill_manual(values = c("R" = "#e3444f", "D" = "#4489e3"))
